...
uart0_rx_intr_handler(void *para)
{
    ...
    */
    ...
    uint8 fifo_len = 0;
    uint8 buf_idx = 0;
	uint32_t uart_intr_status = READ_PERI_REG(UART_INT_ST(uart_no));		//获取uart0中断状态
	...
	{	//接收帧错误中断
		...
		{
			os_printf("UART_FRM_ERR_INT_ST\n");
			WRITE_PERI_REG(UART_INT_CLR(uart_no), UART_FRM_ERR_INT_CLR);		//清除帧错误中断
		}
		//接收full中断
		else if(UART_RXFIFO_FULL_INT_ST == (uart_intr_status & UART_RXFIFO_FULL_INT_ST))
		{
			fifo_len = (READ_PERI_REG(UART_STATUS(UART0))>>UART_RXFIFO_CNT_S)&UART_RXFIFO_CNT;
			os_printf("UART_RXFIFO_FULL_INT_ST,fifo_len = %d\n",fifo_len);		//UART1打印中断状态及FIFO数据长度
			for(buf_idx=0;buf_idx<fifo_len;buf_idx++)//UART0接收串口数据
			{
				uart_tx_one_char(UART0,READ_PERI_REG(UART_FIFO(UART0)) & 0xFF);	//UART0接收到的数据存在FIFO中，
																				//读走FIFO数据(读走什么发送什么)
			}
			WRITE_PERI_REG(UART_INT_CLR(UART0), UART_RXFIFO_FULL_INT_CLR);		//清除full中断
		}
		//接收超时中断
		else if(UART_RXFIFO_TOUT_INT_ST == (uart_intr_status & UART_RXFIFO_TOUT_INT_ST))
		{
			fifo_len = (READ_PERI_REG(UART_STATUS(UART0))>>UART_RXFIFO_CNT_S)&UART_RXFIFO_CNT;
			os_printf("UART_RXFIFO_TOUT_INT_ST,fifo_len = %d\n",fifo_len);		//UART1打印中断状态及FIFO数据长度
			for(buf_idx=0;buf_idx<fifo_len;buf_idx++)//UART0接收串口数据
			{
				uart_tx_one_char(UART0,READ_PERI_REG(UART_FIFO(UART0)) & 0xFF);	//UART0接收到的数据存在FIFO中，
																				//读走FIFO数据(读走什么发送什么)
			}
			WRITE_PERI_REG(UART_INT_CLR(UART0), UART_RXFIFO_TOUT_INT_CLR);		//清除超时中断
		}
		//发送fifo空中断
		else if(UART_TXFIFO_EMPTY_INT_ST == (uart_intr_status & UART_TXFIFO_EMPTY_INT_ST))
		{
			os_printf("UART_TXFIFO_EMPTY_INT_ST\n");							//UART1打印中断状态
			WRITE_PERI_REG(UART_INT_CLR(uart_no), UART_TXFIFO_EMPTY_INT_CLR);	//清除fifo空中断
			CLEAR_PERI_REG_MASK(UART_INT_ENA(UART0), UART_TXFIFO_EMPTY_INT_ENA);//关闭fifo空中断使能
		}
		//接收溢出中断
		else
		{
			os_printf("UART_RXFIFO_OVF_INT_ST\n");								//UART1打印中断状态
		}
		uart_intr_status = READ_PERI_REG(UART_INT_ST(uart_no));					//更新中断状态
		os_printf("uart_intr_status %d\n",uart_intr_status);
	}
}
...
void uart0_sendStr_no_wait(const char *str)
{
    while(*str){
        uart_tx_one_char_no_wait(UART0, *str++);
    }
}
