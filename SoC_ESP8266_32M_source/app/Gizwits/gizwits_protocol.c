...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagLampSwitch)
    {
        info->event[info->num] = EVENT_LampSwitch;
        info->num++;
        dataPoints->valueLampSwitch = gizStandardDecompressionValue(LampSwitch_BYTEOFFSET,LampSwitch_BITOFFSET,LampSwitch_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueLampSwitch != cur->valueLampSwitch)
    {
        GIZWITS_LOG("valueLampSwitch Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(LampSwitch_BYTEOFFSET,LampSwitch_BITOFFSET,LampSwitch_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLampSwitch);
